...
public class CustomLoad : IExposable
{
    ...
    public void GenerateDefs(AmmoDef ammoTemplate, ThingDef bulletTemplate, bool register)
    {
        ...
        if (register)
        {
            ...
            var ammoTakenHashesSet = ShortHashGiver.takenHashesPerDeftype.GetValueOrDefault(Ammo.GetType());
            var bulletTakenHashesSet = ShortHashGiver.takenHashesPerDeftype.GetValueOrDefault(Bullet.GetType());
            if (ammoTakenHashesSet == null)
            {
                ammoTakenHashesSet = [];
                ShortHashGiver.takenHashesPerDeftype.Add(Ammo.GetType(), ammoTakenHashesSet);
            }
            if (bulletTakenHashesSet == null)
            {
                bulletTakenHashesSet = [];
                ShortHashGiver.takenHashesPerDeftype.Add(Bullet.GetType(), bulletTakenHashesSet);
            }
            
            ShortHashGiver.GiveShortHash(Ammo, Ammo.GetType(), ammoTakenHashesSet);
            ShortHashGiver.GiveShortHash(Bullet, Bullet.GetType(), bulletTakenHashesSet);
